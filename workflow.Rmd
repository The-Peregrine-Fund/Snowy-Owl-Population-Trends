...
### 2.2.4 A function to assess goodness-of-fit
```{r,PPCfunc, eval=TRUE, warning=FALSE}
```

### 2.2.5 Check goodness-of-fit
...
### 2.3.1 Print table of parameter estimates
```{r,parest, eval=TRUE, warning=FALSE}
```

### 2.3.2 Derive a weighted population growth rate for all sites combined
...
### 2.3.3 Derive percent change for IUCN Red List Criteria

#### 2.3.3.1 8-year generation time
```{r,perchange1, eval=TRUE, warning=FALSE}
```

#### 2.3.3.2 10.7-year generation time
