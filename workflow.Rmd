...
```{r,autocorr, eval=TRUE, eval=TRUE, warning=FALSE, message=FALSE, fig.width=8, fig.height=10, fig.cap= "Fig. S1. Autocorrelation plot of Snowy Owl nest counts."}
```

## 2.2 Posterior predictive checks to assess goodness-of-fit
...
### 2.2.4 Zero-inflated Poisson distribution
```{r,zip, eval=FALSE, warning=FALSE}
```

### 2.2.5 A function to assess goodness-of-fit
```{r,PPCfunc, eval=TRUE, warning=FALSE}
```

### 2.2.6 Check goodness-of-fit
...
## 2.3 Postprocess the best-fitting model (negative binomial)
...
### 2.3.2 Print table of parameter estimates
```{r,parest, eval=TRUE, warning=FALSE}
```

### 2.3.3 Derive a weighted population growth rate for all sites combined
...
### 2.3.4 Derive percent change for IUCN Red List Criteria
```{r,perchange, eval=TRUE, warning=FALSE}
```

```{r, perchangeplot, fig.width=8, fig.height=5, fig.cap= "Fig. S7. Percent change in abundance of Snowy Owl since 1996."}
```

## 2.4 Similar analysis including sites with shorter time series and site as a random effect
...
### 2.4.1 Load packages and manipulate data
```{r,start, eval=TRUE, warning=FALSE, message=FALSE}
```
...
### 2.4.3 Postprocess and plot population growth
```{r,lambda, eval=TRUE, warning=FALSE}
```
...
### 2.4.4 Postprocess and plot IUCN Criteria as percent change
```{r,perc, eval=TRUE, warning=FALSE}
```

```{r, perchangeplot2, fig.width=8, fig.height=5, fig.cap= "Fig. S9. Percent change in abundance of Snowy Owl since 1996 using all data and a random effect for site."}
```
